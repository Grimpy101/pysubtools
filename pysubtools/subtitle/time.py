...
class Time:
    ...
    """
    def __init__(self, milliseconds: int, framerate: typing.Optional[float] = None) -> None:
    ...
    def frame(self) -> Frame:
        if self.framerate is None:
            raise ValueError("Cannot convert Time to frame: no framerate!")
    ...
    @classmethod
    def from_human_time(cls, milliseconds: int, seconds: int = 0, minutes: int = 0, hours: int = 0) -> 'Time':
    ...
    def from_frame(cls, frame: int, framerate: float) -> 'Time':
        ...
        return cls.from_human_time(milliseconds)
    
    def set_framerate(self, new_framerate: typing.Optional[float]) -> None:
